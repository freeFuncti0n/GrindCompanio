...
void UIManager::process_remote_grind_command(uint8_t action) {
    if (!initialized || !grinding_controller_ || !state_machine || !grind_controller) {
        return;
    }
    ...
    switch (action) {
        case BLE_LIVE_CMD_START_GRIND:
            if (!state_machine->is_state(UIState::READY)) return;
            if (grind_controller->is_active()) return;
            if (current_tab >= 3) return;
            if (hardware_manager) {
                WeightSensor* sensor = hardware_manager->get_weight_sensor();
                if (sensor && sensor->has_hardware_fault()) return;
            }
            grinding_controller_->handle_grind_button();
            break;

        case BLE_LIVE_CMD_STOP_GRIND:
        ...
        case BLE_LIVE_CMD_CONTINUE_PURGE:
            if (state_machine->is_state(UIState::PURGE_CONFIRM)) {
                grinding_controller_->handle_purge_confirm_continue();
            }
            break;

        case BLE_LIVE_CMD_RETURN_IDLE:
            if (state_machine->is_state(UIState::GRIND_COMPLETE) ||
                state_machine->is_state(UIState::GRIND_TIMEOUT)) {
                grinding_controller_->handle_grind_button();
            }
