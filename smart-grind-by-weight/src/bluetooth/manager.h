...
private:
    ...
    // Live telemetry characteristics
    BLECharacteristic* live_telemetry_characteristic;
    BLECharacteristic* live_control_characteristic;
    ...
    unsigned long last_live_telemetry_ms;

    // Optional providers for live telemetry (set from main)
    GrindController* grind_controller_ref;
    HardwareManager* hardware_manager_ref;
    ...
    void handle_live_control_command(BLECharacteristic* characteristic);
    void update_live_telemetry();
    void enqueue_remote_grind_command(uint8_t action);
...
public:
    ...
    /**
     * Wire grind/hardware references for live telemetry streaming
     */
    void set_live_sources(GrindController* grind_controller, HardwareManager* hardware_manager);
    ...
    // Drain a remote grind command queued from BLE task; called by UI task
    bool dequeue_remote_grind_command(uint8_t* out_action);
};
