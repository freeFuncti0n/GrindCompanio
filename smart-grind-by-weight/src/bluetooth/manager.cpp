...
BluetoothManager::BluetoothManager()
    ...
    , live_telemetry_characteristic(nullptr)
    , live_control_characteristic(nullptr)
    ...
    , last_live_telemetry_ms(0)
    , grind_controller_ref(nullptr)
    , hardware_manager_ref(nullptr)
...
void BluetoothManager::enable(unsigned long timeout_ms) {
    ...
    // Create live telemetry service (companion app)
    live_service = ble_server->createService(BLE_LIVE_SERVICE_UUID);
    delay(BLE_INIT_SERVICE_DELAY_MS);

    live_telemetry_characteristic = live_service->createCharacteristic(
        BLE_LIVE_TELEMETRY_CHAR_UUID,
        BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY
    );
    delay(BLE_INIT_CHARACTERISTIC_DELAY_MS);

    live_control_characteristic = live_service->createCharacteristic(
        BLE_LIVE_CONTROL_CHAR_UUID,
        BLECharacteristic::PROPERTY_WRITE
    );
    live_control_characteristic->setCallbacks(this);
    delay(BLE_INIT_CHARACTERISTIC_DELAY_MS);
    ...
    advertising->addServiceUUID(BLE_LIVE_SERVICE_UUID);
...
void BluetoothManager::onWrite(BLECharacteristic* characteristic) {
    ...
    } else if (characteristic == live_control_characteristic) {
        handle_live_control_command(characteristic);
    } else {
...
void BluetoothManager::set_live_sources(GrindController* grind_controller, HardwareManager* hardware_manager) {
    grind_controller_ref = grind_controller;
    hardware_manager_ref = hardware_manager;
}

void BluetoothManager::handle_live_control_command(BLECharacteristic* characteristic) {
    String value = characteristic->getValue();
    if (value.length() == 0) return;
    uint8_t command = static_cast<uint8_t>(value[0]);
    if (command == BLE_LIVE_CMD_ENABLE) {
        live_stream_active = true;
        last_live_telemetry_ms = 0;
        log("Bluetooth Live: Streaming enabled\n");
    } else if (command == BLE_LIVE_CMD_DISABLE) {
        live_stream_active = false;
        log("Bluetooth Live: Streaming disabled\n");
    } else if (command == BLE_LIVE_CMD_START_GRIND ||
               command == BLE_LIVE_CMD_STOP_GRIND ||
               command == BLE_LIVE_CMD_CONTINUE_PURGE ||
               command == BLE_LIVE_CMD_RETURN_IDLE) {
        enqueue_remote_grind_command(command);
    }
}

void BluetoothManager::update_live_telemetry() {
    if (!ble_enabled || !device_connected || !live_stream_active) return;
    if (ota_handler.is_ota_active() || data_export_in_progress) return;
    if (!live_telemetry_characteristic || !grind_controller_ref || !hardware_manager_ref) return;
    if (!grind_controller_ref->is_active()) return;
